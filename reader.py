...
# Disable eager execution so that the legacy code runs on TensorFlow 2
...
class Reader:
    ...
    def __init__(
        ...
        self.context_pad = f"{Common.PAD},{Common.PAD},{Common.PAD}"
    ...
    def process_dataset(self, *row_parts):
        ...
        if not self.is_evaluating and self.config.RANDOM_CONTEXTS:
            ...
            num_contexts_per_example = tf.reduce_min(
                input_tensor=index_of_blank_context
            )
        ...
        target_dense_shape = [
            1,
            tf.maximum(
                tf.cast(self.config.MAX_TARGET_PARTS, dtype=tf.int64),
        ...
        sparse_split_source = tf.sparse.SparseTensor(
            ...
            dense_shape=[
                self.config.MAX_CONTEXTS,
                tf.maximum(
                    tf.cast(self.config.MAX_NAME_PARTS, dtype=tf.int64),
        ...
        path_source_lengths = tf.reduce_sum(
            input_tensor=tf.cast(
                tf.not_equal(dense_split_source, Common.PAD), tf.int32
            ),
            axis=-1,
        )  # (max_contexts)
        ...
        path_lengths = tf.reduce_sum(
            input_tensor=tf.cast(tf.not_equal(dense_split_path, Common.PAD), tf.int32),
            axis=-1,
        )  # (max_contexts)
        ...
        path_target_lengths = tf.reduce_sum(
            input_tensor=tf.cast(
                tf.not_equal(dense_split_target, Common.PAD), tf.int32
            ),
            axis=-1,
        )  # (max_contexts)
        ...
        valid_contexts_mask = tf.cast(
            tf.not_equal(
                tf.reduce_max(input_tensor=path_source_indices, axis=-1)
                + tf.reduce_max(input_tensor=node_indices, axis=-1)
                + tf.reduce_max(input_tensor=path_target_indices, axis=-1),
                0,
            ),
            dtype=tf.float32,
        )
