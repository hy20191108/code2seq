...
class Model:
    ...
    def __init__(self, config: Config):
        ...
        if config.LOAD_PATH:
            ...
            with open(f"{config.TRAIN_PATH}.dict.c2s", "rb") as file:
        ...
        for iteration in range(
            ...
            try:
                ...
                if self.config.BEAM_WIDTH == 0:
                    print(
                        "Accuracy after %d epochs: %.5f"
                        % (self.epochs_trained, results)
                    )
                else:
                    print(f"Accuracy after {self.epochs_trained} epochs: {results}")
                print(
            ...
            try:
                while True:
                    ...
                    true_positive, false_positive, false_negative = (
                        self.update_per_subtoken_statistics(
                            zip(true_target_strings, predicted_strings),
                            true_positive,
                            false_positive,
                            false_negative,
        ...
        for original_name, predicted in results:
            ...
            if self.config.BEAM_WIDTH == 0:
                ...
                if (
                    ...
                    or "".join(filtered_original)
                    == "".join(filtered_predicted_first_parts)
                ):
                ...
                if true_ref in filtered_predicted:
                    ...
                    update = np.concatenate(
                        [
                            ...
                            np.ones(
                                self.config.BEAM_WIDTH - index_of_correct,
                                dtype=np.int32,
                            ),
                        ]
                    )
    ...
    def decode_outputs(
        ...
        outputs, final_states, final_sequence_lengths = (
